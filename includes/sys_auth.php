...
function set_password($uid, $password) {
  return sql_query("UPDATE `User` SET `password` = '" . sql_escape(crypt($password, CRYPT_ALG . '$' . generate_salt(16) . '$')) . "', `password_recovery_token`=NULL WHERE `UID` = " . intval($uid) . " LIMIT 1");
}
...
function verify_password($password, $salt, $uid = false) {
  ...
  if ($correct && substr($salt, 0, strlen(CRYPT_ALG)) != CRYPT_ALG && $uid) {
    ...
    sql_query("UPDATE `User` SET `password` = '" . sql_escape(crypt($password, CRYPT_ALG . '$' . generate_salt() . '$')) . "' WHERE `UID` = " . intval($uid) . " AND `password` = '" . sql_escape($salt) . "' LIMIT 1");
  }
  return $correct;
}
